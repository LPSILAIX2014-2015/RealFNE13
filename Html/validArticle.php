...
$state = $connec->prepare(
    "SELECT P.*, P.PTYPE, U.NAME, U.SURNAME, DATE_FORMAT(P.PDATE, '%d/%m/%Y') AS PDATE
     FROM POST P, USER U
     WHERE P.WRITER_ID = U.ID AND P.STATUS=0 AND P.PTYPE='ARTICLE'
     ORDER BY ID DESC"
);
$state->execute();
...
$state = $connec->prepare(
    "SELECT P.*, P.PTYPE, U.NAME, U.SURNAME, U.ASSOCIATION_ID, DATE_FORMAT(P.PDATE, '%d/%m/%Y') AS PDATE
     FROM POST P, USER U
     WHERE P.WRITER_ID = U.ID AND P.STATUS=0 AND P.PTYPE='ARTICLE'
     ORDER BY id DESC"
);
