<?php
    ...
    if ( !empty($_POST))
    {
        ...
        $sql = "UPDATE USER SET LOGIN = ?, PASSWORD = ?, ADRESS = ?, CP = ?, THEME_ID = ?, THEME_INTEREST_ID= ?, PROFESSION = ?, PRESENTATION = ?, PROFESSION2 = ?, THEME_DETAILS = ? WHERE MAIL = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($login, $mdp, $address, $cp, $theme, $theme_interest, $profession, $presentation, $profession2, $details, $user_mail));
